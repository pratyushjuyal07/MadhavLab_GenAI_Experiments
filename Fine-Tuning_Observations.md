# Project Analysis: Final Fine-Tuning and Inference Observations
...
## 3. Fine-Tuning Attempt 1: The Broken Output

### Methodology
Initial fine-tuning (Run Beta) applied a high learning rate of 1e-4 and an aggressive adapter scaling factor (`lora_alpha=32`, `r=16`) over 5 training epochs utilizing the 5-file chunked dataset.
...
| **Grand Ensemble** | A grand Indian traditional raga-styled ensemble performance... | 10s | Total acoustic failure consisting entirely of clipping noise, audio stuttering, and systemic distortion. | Model suffered from severe overfitting and weight explosion. The hyperparameter configuration forced memorization of raw waveform data boundaries instead of stylistic abstraction. |

### Final Attempt 1 Observations
This iteration highlighted the sensitivity of autoregressive audio decoders to aggressive parameter adjustments. Excessive learning rates and improper LoRA alpha-to-rank balancing destabilize the pre-trained latent space, resulting in catastrophic failure of the generation pipeline.

---

## 4. Fine-Tuning Attempt 2: The Successful Fix

### Methodology
Second fine-tuning iteration (Run Gamma) introduced stabilized hyperparameters to protect pre-trained weights. The learning rate was lowered to 2e-5, the LoRA alpha-to-rank ratio was optimized to 1:1 (`lora_alpha=16`, `r=16`), and total training duration was capped at 3 epochs to avoid overfitting.
