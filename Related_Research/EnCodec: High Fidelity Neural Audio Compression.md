...
* **Deterministic Gradient Balance:** The paper's gradient loss balancer can serve as an indirect empirical tracking mechanism. Monitoring the stabilized component weights ($\lambda_t, \lambda_f, \lambda_g$) during fine-tuning provides clear insights into whether our adapter's optimization step is actively prioritizing time-domain wave tracking (rhythmic accuracy) or frequency-domain spectrogram contours (melodic nuance).
